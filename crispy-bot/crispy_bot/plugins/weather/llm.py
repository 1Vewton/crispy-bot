...
如果数据未提供就不要把这个信息加到你的回答中！(e.g. 如果云量的数据没有提供就不要把云量加到你的回答中)
...
- 最近一小时的降水量: {weather_result["now"]["precip"]}毫米
- 能见度: {weather_result["now"]["vis"]}公里
- 测量时间: {weather_result["now"]["obsTime"]}
- 云量: {weather_result["now"].get("cloud", "数据未提供")}%
- 露点温度: {weather_result["now"].get("dew", "数据未提供")}摄氏度
# 参考信息
- **蒲福风级 0**
  - 术语(ZH): 无风
  - 术语(EN): Calm
...
# 指示
你是人工智能助手Crispy，你需要用轻松活泼的语气说明以下信息(**不要遗漏任何关键信息!**)，
你可以适当使用喵或者emoji来提高语气的亲和性，但不要加太多，容易让用户觉得烦，并且你可以给出对未来的天气的评价以及给用户的建议
注意你只回一段对话，所以说不要加"还有什么要问的"这样的话语在你的回答的最后面。
如果数据未提供就不要把这个信息加到你的回答中！(e.g. 如果某一天的日出时间的数据没有提供就不要把那一天的日出时间加到你的回答中)
**你的前端不支持markdown语法解析，所以不要使用MD格式回答内容!**
这里的所有风级都采用蒲福风级
# {position}未来一段时间的天气
{
...
f"""
## {i["fxDate"]}
- 日出时间: {i.get("sunrise", "数据未提供")}
- 日落时间: {i.get("sunset", "数据未提供")}
- 月升时间: {i.get("moonrise", "数据未提供")}
- 月落时间: {i.get("moonset", "数据未提供")}
- 月相: {i["moonPhase"]}
- 最高温度: {i["tempMax"]}摄氏度
- 最低温度: {i["tempMin"]}摄氏度
- 白天天气状况: {i["textDay"]}
- 夜间天气状况: {i["textNight"]}
- 白天风向角度: {i["wind360Day"]}度
- 白天风向: {i["windDirDay"]}
- 白天风力等级: {i["windScaleDay"]}
- 白天风速: {i["windSpeedDay"]}公立/小时
- 夜间风向角度: {i["wind360Night"]}度
- 夜间风向: {i["windDirNight"]}
...
## 紫外线等级
- **1级（弱）**：紫外线指数 0-2，无需防护。  
- **2级（中等）**：紫外线指数 3-4，建议戴遮阳帽、太阳镜。  
- **3级（强）**：紫外线指数 5-6，需涂抹防晒霜，穿防晒衣物。  
- **4级（很强）**：紫外线指数 7-9，避免正午外出，加强防护。  
- **5级（极强）**：紫外线指数 ≥10，尽量不外出，防护措施务必到位。
"""
    response = await litellm.acompletion(
        model=f"{llm_provider}/{llm_model_name}",
        base_url=llm_base_url,
